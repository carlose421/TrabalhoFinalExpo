...
import { useRouter } from 'expo-router';
	...
	export default function NatacaoScreen() {
		...
		watchRef.current = await Location.watchPositionAsync(
			...
			(loc) => {
				if (coords) {
					...
					const R = 6371; // km
					const dLat = toRad(loc.coords.latitude - coords.latitude);
					const dLon = toRad(loc.coords.longitude - coords.longitude);
					const a =
					...
					const d = R * c;
					setDistance(prev => prev + d); // km
				}
				setCoords({ latitude: loc.coords.latitude, longitude: loc.coords.longitude });
				setSpeed(loc.coords.speed ? loc.coords.speed * 3.6 : null); // km/h
			}
		);
	};
	...
	return (
		<View style={styles.container}>
			<View style={styles.header}>
				...
				<Text style={styles.headerTitle}>Natação</Text>
				<Ionicons name="water" size={28} color="#222" />
			</View>
			{!started ? (
				<>
					...
					<TouchableOpacity style={styles.startBtn} onPress={startSwim}>
				...
				<>
					...
					<View style={styles.btnRow}>
						<TouchableOpacity style={styles.pauseBtn} onPress={pauseSwim}><Text style={styles.pauseBtnText}>Pausar</Text></TouchableOpacity>
						<TouchableOpacity style={styles.finishBtn} onPress={finishSwim} activeOpacity={0.7}>
